...
public class PerfScoreFlowController extends BaseController
{
     ...
     * 查询评分流程列表
     */
    @PreAuthorize("@ss.hasPermi('perf:scoreFlow:list')")
    @GetMapping("/list")
     ...
     * 导出评分流程列表
     */
    @PreAuthorize("@ss.hasPermi('perf:scoreFlow:export')")
     ...
     * 获取评分流程详细信息
     */
    @PreAuthorize("@ss.hasPermi('perf:scoreFlow:query')")
    @GetMapping(value = "/{id}")
    public AjaxResult getInfo(@PathVariable("id") Long id)
    {
        return success(perfScoreFlowService.selectPerfScoreFlowById(id));
    }

    /**
     * 新增评分流程
     */
    @PreAuthorize("@ss.hasPermi('perf:scoreFlow:add')")
     ...
     * 修改评分流程
     */
    @PreAuthorize("@ss.hasPermi('perf:scoreFlow:edit')")
     ...
     * 删除评分流程
     */
    @PreAuthorize("@ss.hasPermi('perf:scoreFlow:remove')")
    @Log(title = "评分流程", businessType = BusinessType.DELETE)
	@DeleteMapping("/{ids}")
    public AjaxResult remove(@PathVariable Long[] ids)
    {
        return toAjax(perfScoreFlowService.deletePerfScoreFlowByIds(ids));
    }
}
