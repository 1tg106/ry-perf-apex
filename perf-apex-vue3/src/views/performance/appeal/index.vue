  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:appeal:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['system:appeal:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="appealList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:appeal:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['system:appeal:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { listAppeal, getAppeal, delAppeal, addAppeal, updateAppeal } from "@/api/performance/appeal"
...
function handleSelectionChange(selection) {
  ids.value = selection.map(item => item.appealId)
...
function handleUpdate(row) {
  reset()
  const _appealId = row.appealId || ids.value
  getAppeal(_appealId).then(response => {
...
function handleDelete(row) {
  const _appealIds = row.appealId || ids.value
  proxy.$modal.confirm('是否确认删除绩效申诉编号为"' + _appealIds + '"的数据项？').then(function() {
    return delAppeal(_appealIds)
  }).then(() => {
    getList()
...
function handleExport() {
  proxy.download('system/appeal/export', {
    ...queryParams.value
