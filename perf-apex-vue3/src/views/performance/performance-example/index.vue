  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="performanceList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:performance:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['system:performance:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { listPerformance, getPerformance, delPerformance, addPerformance, updatePerformance } from "@/api/performance/performance"
...
function handleUpdate(row) {
  reset()
  const _performanceId = row.performanceId || ids.value
  getPerformance(_performanceId).then(response => {
...
function submitForm() {
  proxy.$refs["performanceRef"].validate(valid => {
    if (valid) {
      if (form.value.performanceId != null) {
...
function handleDelete(row) {
  const _performanceIds = row.performanceId || ids.value
  proxy.$modal.confirm('是否确认删除绩效实例编号为"' + _performanceIds + '"的数据项？').then(function() {
    return delPerformance(_performanceIds)
  }).then(() => {
    getList()
...
function handleExport() {
  proxy.download('system/performance/export', {
    ...queryParams.value
