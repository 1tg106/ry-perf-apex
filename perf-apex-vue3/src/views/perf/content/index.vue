    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['perf:content:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['perf:content:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="contentList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['perf:content:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['perf:content:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { listContent, getContent, delContent, addContent, updateContent } from "@/api/perf/content"
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除绩效内容编号为"' + _ids + '"的数据项？').then(function() {
    return delContent(_ids)
  }).then(() => {
    getList()
...
function handleExport() {
  proxy.download('perf/content/export', {
    ...queryParams.value
