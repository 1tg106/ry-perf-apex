<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['perf:scoreFlow:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['perf:scoreFlow:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['perf:scoreFlow:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="scoreFlowList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['perf:scoreFlow:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['perf:scoreFlow:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="scoreFlowRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="绩效ID" prop="performanceId">
...
<script setup name="ScoreFlow">
import { listScoreFlow, getScoreFlow, delScoreFlow, addScoreFlow, updateScoreFlow } from "@/api/perf/scoreFlow"

const { proxy } = getCurrentInstance()

const scoreFlowList = ref([])
...
function getList() {
  loading.value = true
  listScoreFlow(queryParams.value).then(response => {
    scoreFlowList.value = response.rows
...
function handleUpdate(row) {
  ...
  getScoreFlow(_id).then(response => {
...
/** 提交按钮 */
function submitForm() {
  proxy.$refs["scoreFlowRef"].validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateScoreFlow(form.value).then(response => {
          proxy.$modal.msgSuccess("修改成功")
          open.value = false
          getList()
        })
      } else {
        addScoreFlow(form.value).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除评分流程编号为"' + _ids + '"的数据项？').then(function() {
    return delScoreFlow(_ids)
  }).then(() => {
    getList()
...
function handleExport() {
  proxy.download('perf/scoreFlow/export', {
    ...queryParams.value
  }, `scoreFlow_${new Date().getTime()}.xlsx`)
}
